...
func ThreadHandler(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return ErrorHTML(w, r, "Неправильная тема", "Такой темы нету", http.StatusBadRequest, err)
	}
...
// Guest must register	
	...
	if tree == nil {
		return ErrorHTML(w, r, "Тема не найдена", "Темы которую вы ищете уже нет", http.StatusNotFound, errors.New("tree was nil"))
	}
...
// Guest must register	
	...
	if tree == nil {
		return ErrorHTML(w, r, "Форум не найден", "Форума который вы ищете уже нет.", http.StatusNotFound, errors.New("tree was nil"))
	}
...
// Guest must register	
	if u.Guest() {
		w.WriteHeader(http.StatusBadRequest)
		return errors.New("Извините, необходима регистрация")
	}
